...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
        ...
        ! JVR Modification TODO: change this?
    ...
    real(dl) :: grhov_t, rhonu, grhonu, a, xi_interaction, w_eff, w_dark_energy ! JVR Modification - added xi_interaction and w_eff
    real(dl), intent(out) :: densities(8,n)
    integer nu_i,i

    do i=1, n
        a = a_arr(i)
        call this%CP%DarkEnergy%BackgroundDensityAndPressure(this%grhov, a, grhov_t, w_dark_energy)
        grhonu = 0
        ! JVR Modification Begins
        xi_interaction = this%CP%DarkEnergy%xi_interaction
        w_eff = w_dark_energy + xi_interaction / 3._dl
        if (this%CP%Num_Nu_massive /= 0) then
        ...
        densities(3,i) = this%grhoc * a + xi_interaction * this%grhov * a * (1 - a**(-3._dl*w_eff)) / (3._dl * w_eff)
    ...
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) grhoa2, rhonu, xi_interaction, w_eff ! JVR Modification - added xi_interaction and w_eff
    integer nu_i
